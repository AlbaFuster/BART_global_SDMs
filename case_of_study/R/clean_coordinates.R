...
#' @examples
#' my_data <- read.csv("spatial_data.csv")
#' unique_data <- remove_duplicate_points(my_data, coords = c("decimalLongitude", "decimalLatitude"))
...
#' library(spData)
#' data("nc")
#' # Filter points falling within a specific polygon
#' filtered_points <- remove_points_poly(nc, sf_poly = nc[1, ], overlapping = TRUE)
#' # Filter points falling outside a specific polygon
#' filtered_points <- remove_points_poly(nc, sf_poly = nc[1, ], overlapping = FALSE)
