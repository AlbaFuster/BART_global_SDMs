...
cutoff <- list()
for(i in 1:50) {
  cutoff[[i]] <- optimalCutoff(data_random_17[[i]]$ocurrences,
                               data_random_17[[i]]$fit)
}
...
for(i in 1:50) {
  print(i)
  sensitivity_intra[[i]] <- caret::sensitivity(conf_matrix[[i]])
  specificity_intra[[i]] <- caret::specificity(conf_matrix[[i]])
  accuracy_intra[[i]] <- Metrics::accuracy(data_random_17[[i]]$ocurrences,
                                  data_random_17[[i]]$fit_ocurrence)
}
...
for(j in 1:50) {
  ...
  tryCatch({
    for(i in 1:k) {
      sensitivity[[i]] <- caret::sensitivity(confusion_all[[j]][[i]])
      specificity[[i]] <- caret::specificity(confusion_all[[j]][[i]])
      accuracy[[i]] <- Metrics::accuracy(data_all_2[[j]][data_all_2[[j]]$time == i,]$ocurrences,
